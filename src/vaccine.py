...
def vaccine(vax_deduplicated):
    window_spec = Window.partitionBy("person_id").orderBy("person_id", "vax_date")
    vax_with_row_number = vax_deduplicated.withColumn("number", F.row_number().over(window_spec))
    ...
    vax_transactions = vax_deduplicated.groupBy("person_id") \
