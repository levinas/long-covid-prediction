def distinct_vax_person_test(concept_set_members, 
                        drug_exposure_test, 
                        # manifest_safe_harbor_test,
                        procedure_occurrence_test):
    subquery1 = concept_set_members.alias("cs").join(
        drug_exposure_test.alias("de"), col("cs.concept_id") == col("de.drug_concept_id"), "inner"
    # ).join(
    #     manifest_safe_harbor_test.alias("m"), col("de.data_partner_id") == col("m.data_partner_id"), "inner"
    ).filter(
    ...
    subquery2 = procedure_occurrence_test.alias("po"
    # ).join(
    #     manifest_safe_harbor_test.alias("m"), col("po.data_partner_id") == col("m.data_partner_id"), "inner"
    ).filter(
