def observation_period_features(observation_period, silver):
    df_end = observation_period.select('person_id', 'observation_period_end_date')
    df = keep_last_occurrence(df_end, 'person_id', 'observation_period_end_date')
    df = silver.select('person_id', 'covid_index').join(df, on='person_id', how='left')
