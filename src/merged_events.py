# typical runtime: 1m20s

# def merged_events(concept, condition_occurrence, device_exposure, drug_exposure, observation, procedure_occurrence, note, note_nlp, visit_occurrence):
def merged_events(concept, condition_occurrence, drug_exposure, observation, procedure_occurrence, visit_occurrence):
    df1 = condition_occurrence.select('person_id', 
    ...
    # df2 = device_exposure.select('person_id', 
    ...
    df3 = drug_exposure.select('person_id', 
    ...
    df4 = observation.select('person_id', 
    ...
    df5 = procedure_occurrence.select('person_id', 
    ...
    # df6 = note.select('person_id', 'note_id', 'note_date', 'note_type_concept_id', 'note_type_concept_name') \
    #     .join(note_nlp.select('note_id', 'note_nlp_concept_id', 'note_nlp_concept_name'), on='note_id') \
    #     .select('person_id', 
    ...
    df7 = visit_occurrence.select('person_id', 
