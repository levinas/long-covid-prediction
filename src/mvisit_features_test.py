def mvisit_features_test(microvisits_to_macrovisits_test, silver_test):
    df = microvisits_to_macrovisits_test.withColumn('visit_length', 
        datediff(col('visit_end_date'), col('visit_start_date'))) \
        .select('person_id', 'visit_start_date', 'visit_end_date', 'visit_length')        
    df_covid_index = silver_test.select('person_id', 'covid_index')
