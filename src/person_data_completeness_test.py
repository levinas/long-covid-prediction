def person_data_completeness_test(person_events_test, person_labs_test):
    cat_domains = ['Condition', 'Drug', 'Procedure', 'Visit', 'Observation']
    df_cat = person_events_test.groupBy('person_id').pivot('domain_id', cat_domains).agg(count('date'))

    lab_domains = ['Measurement']
    df_lab = person_labs_test.groupBy('person_id').agg(countDistinct('measurement_date').alias('Measurement'))
